...
import j.jave.framework.validate.JFileExtensionValidator;
import j.jave.framework.validate.JFileNameValidator;
...
public class JDefaultLocalFilePathStrategy {
	...
	public URI resolveURI(JHierarchicalPath hierarchicalPath) {
		...
		if(JUtils.isNullOrEmpty(extensionWithDot)){
		...
		// find by date string.
		if(parentFile!=null){
			String today=JUtils.format(new Date());
