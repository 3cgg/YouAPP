...
public class JDataSourceTransactionManager extends JAbstractPlatformTransactionManager
		implements JResourceTransactionManager {
	 ...
	 * @see #setDataSource
	 */
	public JDataSourceTransactionManager() {
	 ...
	 */
	public JDataSourceTransactionManager(DataSource dataSource) {
...
//		else 
	...
	protected Object doGetTransaction() {
		JDataSourceTransactionObject txObject = new JDataSourceTransactionObject();
		txObject.setSavepointAllowed(isNestedTransactionAllowed());
		JConnectionHolder conHolder =
				(JConnectionHolder) JTransactionSynchronizationManager.getResource(this.dataSource);
	...
	protected boolean isExistingTransaction(Object transaction) {
		JDataSourceTransactionObject txObject = (JDataSourceTransactionObject) transaction;
	 ...
	 */
	@Override
	protected void doBegin(Object transaction, JTransactionDefinition definition) {
		JDataSourceTransactionObject txObject = (JDataSourceTransactionObject) transaction;
		Connection con = null;
		...
		try {
			if (txObject.getConnectionHolder() == null ||
				...
				txObject.setConnectionHolder(new JConnectionHolder(newCon), true);
			}

			txObject.getConnectionHolder().setSynchronizedWithTransaction(true);
			con = txObject.getConnectionHolder().getConnection();

			Integer previousIsolationLevel = JDataSourceUtils.prepareConnectionForTransaction(con, definition);
			...
			if (timeout != JTransactionDefinition.TIMEOUT_DEFAULT) {
				txObject.getConnectionHolder().setTimeoutInSeconds(timeout);
			}

			// Bind the session holder to the thread.
			if (txObject.isNewConnectionHolder()) {
				JTransactionSynchronizationManager.bindResource(getDataSource(), txObject.getConnectionHolder());
			}
		}

		catch (Throwable ex) {
			if (txObject.isNewConnectionHolder()) {
				JDataSourceUtils.releaseConnection(con, this.dataSource);
				txObject.setConnectionHolder(null, false);
			}
			throw new JCannotCreateTransactionException("Could not open JDBC Connection for transaction", ex);
		}
	}
...
//		txObject.setConnectionHolder(null);
		JConnectionHolder conHolder =txObject.getConnectionHolder();
//		conHolder=(JConnectionHolder)
...
//		TransactionSynchronizationManager.bindResource(this.dataSource, conHolder);
	...
	@Override
	protected void doCommit(JDefaultTransactionStatus status) {
		JDataSourceTransactionObject txObject = (JDataSourceTransactionObject) status.getTransaction();
		...
		try {
			...
			throw new JTransactionSystemException("Could not commit JDBC transaction", ex);
		}
	}

	@Override
	protected void doRollback(JDefaultTransactionStatus status) {
		JDataSourceTransactionObject txObject = (JDataSourceTransactionObject) status.getTransaction();
		Connection con = txObject.getConnectionHolder().getConnection();
		if (status.isDebug()) {
		...
		try {
			...
			throw new JTransactionSystemException("Could not roll back JDBC transaction", ex);
		}
	}

	@Override
	protected void doSetRollbackOnly(JDefaultTransactionStatus status) {
		JDataSourceTransactionObject txObject = (JDataSourceTransactionObject) status.getTransaction();
		if (status.isDebug()) {
...
//		}
	 ...
	 */
	private static class JDataSourceTransactionObject extends JJdbcTransactionObjectSupport {

		private boolean newConnectionHolder;

		private boolean mustRestoreAutoCommit;

		public void setConnectionHolder(JConnectionHolder connectionHolder, boolean newConnectionHolder) {
