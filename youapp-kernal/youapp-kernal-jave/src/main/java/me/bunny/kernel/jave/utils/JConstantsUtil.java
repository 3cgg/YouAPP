...
public class JConstantsUtil {
	 ...
	 */
	public JConstantsUtil(Class<?> clazz) {
		JAssert.notNull(clazz);
		this.className = clazz.getName();
		Field[] fields = clazz.getFields();
		for (Field field : fields) {
			if (JClassUtils.isPublicStaticFinal(field)) {
	 ...
	 * @see #asObject
	 * @throws JConstantException if the field name wasn't found
	 * or if the type wasn't compatible with Number
	 */
	public Number asNumber(String code) throws JConstantException {
		Object obj = asObject(code);
		if (!(obj instanceof Number)) {
			throw new JConstantException(this.className, code, "not a Number");
		}
		return (Number) obj;
	}
	 ...
	 * @throws JConstantException if the field name wasn't found
	 */
	public String asString(String code) throws JConstantException {
	 ...
	 * @throws JConstantException if there's no such field
	 */
	public Object asObject(String code) throws JConstantException {
		JAssert.notNull(code, "Code must not be null");
		String codeToUse = code.toUpperCase(Locale.ENGLISH);
		Object val = this.fieldCache.get(codeToUse);
		if (val == null) {
			throw new JConstantException(this.className, codeToUse, "not found");
		}
		return val;
	}
	 ...
	 * @throws JConstantException if the value wasn't found
	 */
	public String toCode(Object value, String namePrefix) throws JConstantException {
		String prefixToUse = (namePrefix != null ? namePrefix.trim().toUpperCase(Locale.ENGLISH) : "");
		...
		throw new JConstantException(this.className, prefixToUse, value);
	}
	 ...
	 * @see #propertyToConstantNamePrefix
	 */
	public String toCodeForProperty(Object value, String propertyName) throws JConstantException {
	 ...
	 * @throws JConstantException if the value wasn't found
	 */
	public String toCodeForSuffix(Object value, String nameSuffix) throws JConstantException {
		String suffixToUse = (nameSuffix != null ? nameSuffix.trim().toUpperCase(Locale.ENGLISH) : "");
		for (Map.Entry<String, Object> entry : this.fieldCache.entrySet()) {
		...
		throw new JConstantException(this.className, suffixToUse, value);
	}
